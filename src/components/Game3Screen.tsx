...
export default function Game3Screen({ state, setState }: Game3ScreenProps) {
  ...
  const handleSetup = () => {
    ...
    if (B === 0) {
      setErrorMsg('At least 1 prize is required to create groups.');
      return;
    }
    
    if (A < B) {
      setErrorMsg(`Not enough players. Need at least ${B} players for ${B} prizes.`);
      return;
    }
    ...
    <div className="w-full h-full flex flex-col items-center p-8 overflow-y-auto">
        ...
        <div className="bg-gray-800/80 p-8 rounded-2xl max-w-2xl w-full border border-gray-700 shadow-xl text-center">
          ...
          <div className="grid grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-gray-900/50 p-6 rounded-xl border border-gray-700">
              ...
              <div className="text-gray-400">Prizes (Groups)</div>
            </div>
          </div>
      ...
      {phase === 'elimination_reveal' && (
        <div className="bg-gray-800/80 p-8 rounded-2xl max-w-3xl w-full border border-gray-700 shadow-xl text-center">
          <h2 className="text-2xl font-bold mb-6 text-red-400">Group Draw Results</h2>
          
          <div className="mb-8">
            <p className="text-xl mb-2">Successfully split into <span className="font-bold text-yellow-400">{groups.length}</span> arena groups.</p>
            <p className="text-xl">All <span className="font-bold text-blue-400">{state.players.length}</span> players are enrolled!</p>
          </div>

          <div className="mb-8 p-4 bg-green-900/30 border border-green-700/50 rounded-lg text-green-300">
            {groups.length > 1 ? (
              <span>
                The first {groups.length - 1} groups have <b>{groups[0]?.players.length}</b> players each.<br/>
                The final group has <b>{groups[groups.length - 1]?.players.length}</b> players.
              </span>
            ) : (
              <span>All players are placed in a single unified group.</span>
            )}
          </div>
      ...
      {phase === 'group_selection' && (
        <div className="w-full max-w-6xl">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold">Select Tournament Group</h2>
            {allFinished && (
              <button
            ...
            {groups.map((group, idx) => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-xl font-bold text-white">Group {idx + 1}</h3>
                ...
                {group.winner ? (
                  <div className="flex-1 flex flex-col items-center justify-center py-4">
                    ...
                    <div className="mt-2 text-yellow-500 font-medium">Has Won!</div>
                  </div>
                ) : (
                  <div className="flex-1 flex flex-col justify-between">
                    <div className="text-gray-400 text-sm mb-4">
                      Active Round: {group.currentRound.length} players
                    </div>
  ...
  if (group.currentRound.length === 1) {
    return (
      <div className="flex flex-col items-center justify-center h-full max-w-2xl w-full bg-gray-800/80 rounded-2xl border border-yellow-500/50 p-12 shadow-2xl">
        ...
        <h2 className="text-4xl font-bold text-yellow-400 mb-8 text-center">Group Winner!</h2>
        ...
        <div className="mt-8 text-center">
          ...
          <p className="text-lg text-yellow-500 mt-4">Prize: {group.prize.name}</p>
        </div>
  ...
  return (
    <div className="w-full max-w-5xl flex flex-col items-center">
      <div className="flex justify-between items-center w-full mb-8">
        ...
        <h2 className="text-2xl font-bold text-orange-400">Tournament Round: {group.currentRound.length} players</h2>
                ...
                {pair.length === 2 ? (
                  <>
                    ...
                    {result !== undefined ? (
                      ...
                      <span className="text-sm text-orange-400 font-bold animate-bounce">Fighting...</span>
                    ) : (
                      <button
                  ...
                  <span className="text-sm text-gray-400 bg-gray-800 px-3 py-1 rounded-full">Bye</span>
                )}
              </div>
